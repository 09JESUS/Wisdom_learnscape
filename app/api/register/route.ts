...
export async function POST(req: Request) {
  try {
    ...
    // 1️⃣ Check if learner email already exists
    ...
    // 2️⃣ Check if parent contact number exists
    ...
    // 4️⃣ Insert learner into users table
    const [learnerUser]: any = await db.query(
      `
      INSERT INTO users
      (name, surname, email, phone, password_hash, role)
      VALUES (?, ?, ?, ?, ?, ?)
      `,
      [
        learner.first_name,
    ...
    // 5️⃣ Insert learner into learners table
    await db.query(
      `
      INSERT INTO learners
      (user_id, first_name, last_name, nickname, gender, phone_number, school, subject)
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
      `,
      [
        learnerUserId,
    ...
    // 6️⃣ Insert parent into users table
    ...
    // 7️⃣ Insert parent into parents table
    await db.query(
      `
      INSERT INTO parents
      (user_id, title, initials, surname, contact_number)
      VALUES (?, ?, ?, ?, ?)
      `,
      [
        parentUserId,
